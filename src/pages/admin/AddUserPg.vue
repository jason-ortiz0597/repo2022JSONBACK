  ...
  <div class="row q-pa-md justify-center">
    <div class="col-xs-10 col-sm-6 col-md-5 col-lg-5 col-xl-2">
      <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-lg q-mt-md">
        ...
        <selAdd v-model="typeUser" v-bind:model="'typeUser'" v-bind:label="'Tipo de usuario'"
          v-bind:noData="'Sin tipos de usuario, agregue uno'" v-bind:Icon="'people_alt'" v-bind:data="
            userStore.TypeUser.map((item) => ({
              label: item.name,
              value: item._id,
            }))
          " @myDialog="myFunction" />
        <selAdd v-model="Points" v-bind:model="'Points'" v-bind:label="'Seleccione Punto Sii-Pi'"
          v-bind:noData="'Sin datos, agregue uno'" v-bind:Icon="'storefront'" v-bind:data="
        ...
        <q-input filled v-model="first_name" label="Nombre completo *" hint="Mínimo 3, Máximo 30 caracteres" lazy-rules
          :rules="[
            (val) =>
              (val && val.length > 2) || 'Nombre(s), Mínimo 3 caracteres',
            (val) => val.length < 31 || 'Nombre(s), Máximo 30 caracteres',
          ]" />
        <q-input filled v-model="last_name" label="Apellido completo *" hint="Mínimo 3, Máximo 30 caracteres" lazy-rules
          :rules="[
            (val) => (val && val.length > 2) || 'Mínimo 6 caracteres',
            (val) => val.length < 31 || 'Máximo 30 caracteres',
          ]" />
        <q-input filled v-model="doc_id" label="Documento de identificación *" hint="Mínimo 5, Máximo 12 caracteres"
          lazy-rules :rules="[
        ...
        <q-input filled v-model="email" type="email" label="Tu email *" hint="Mínimo 5, Máximo 30 caracteres" lazy-rules
          :rules="[
            (val) => (val && val.length > 4) || 'Mínimo 6 caracteres',
            (val) => val.length < 31 || 'Máximo 50 caracteres',
          ]" />

        <q-input filled v-model="username" label="Nombre de usuario *" hint="Mínimo 5, Máximo 20 caracteres" lazy-rules
          :rules="[
            (val) =>
              (val && val.length > 4) ||
      ...
      <q-btn to="/signin" icon="person" flat label="Login" class="col-4 q-mt-xs" />
